...
import { LOREM_TITLE_TEMPLATES } from './lorem-title-templates';
...
export const COMMON_TITLE_TEMPLATES: Record<GenerationLanguage, Record<TitleLengthPreset, string[]>> = {
  ru: {
    ...
    sm: [
      ...
      'Как повысить {result} через {acc}',
      'Быстрая проверка {gen}',
      'Почему {nom} становится важнее',
      'Как избежать {problem} в {prep}',
      'Что меняется в {process}',
      'Короткий гид о {prep}',
      'Как переосмыслить {acc}',
      'Почему {tool} влияет на {result}',
      'Что нужно знать о {prep}',
      'Как {context} помогает улучшить {result}',
    ],
    md: [
      ...
      'Совершенствование {gen}: {context} и обратная связь',
      ...
      'Почему старые подходы в {prep} больше не работают',
      'Как {nom} меняет привычные процессы и решения',
      'Что помогает быстрее разобраться в {prep} без лишней теории',
      'Как избежать {problem} и улучшить {result}',
      'Как {tool} помогает снизить риск {problem}',
      'Что важно учитывать в {process}, чтобы улучшить {result}',
      'Как {context} помогает принимать более уверенные решения',
    ],
    lg: [
      ...
      'Улучшение {gen} как основа для устойчивого результата и понятной командной работы',
      ...
      'Как работа {withInstr} помогает защититься от {problem} и быстрее улучшать {result}',
      'Что происходит в {process}, когда команда перестает полагаться только на интуицию',
      ...
      'Как {tool} помогает команде лучше понимать ограничения и совершенствовать {result}',
    ],
    xlg: [
      ...
      '{context}: как улучшить {result} за счет понятной системы решений, регулярного применения и внимательной работы с ограничениями',
      ...
      'Как системная работа {withInstr} помогает команде лучше понимать ограничения, быстрее избавляться от {problem} и постепенно улучшать {result}',
      'Что важно изменить в представлении о {prep}, чтобы решения перестали быть случайными, а развитие стало последовательным и измеримым',
      'Как переосмыслить {acc}, если старые процессы уже не помогают команде двигаться быстрее, сохранять качество и контролировать {result}',
      'Почему {tool} приобретает ценность только тогда, когда команда ясно понимает проблемы в {process} и умеет работать с ними системно',
    ],
  },
  ...
  en: {
    ...
    sm: [
      ...
      'How to avoid {problem} in {topic}',
      'What is changing in {process}',
      'Short guide to {topic}',
      'How to rethink {topic}',
      'Why {tool} affects {result}',
    ...
    md: [
      ...
      'Why {problem} makes it harder to improve {result}',
      'How {tool} helps reduce the risk of {problem}',
      'What matters in {process} when teams want better {result}',
      'How {context} helps teams make more confident decisions',
    ],
    lg: [
      ...
      'How {context} helps the team detect problems before they become critical',
      ...
      'How working with {topic} helps detect {problem} earlier and improve {result} faster',
      'What happens in {process} when the team stops relying only on intuition',
      'How to turn work with {topic} into a clear system',
      'Why {topic} development starts with clear goals, not with tools',
      'How {tool} helps teams understand constraints and improve {result}',
    ],
    xlg: [
      ...
      'How systematic work with {topic} helps the team understand constraints, detect {problem} faster and gradually improve {result}',
      'What should change in the approach to {topic} so decisions become less random and development becomes more consistent',
      'How to rethink {topic} when old processes no longer help the team move faster, preserve quality and control {result}',
      'Why {tool} becomes useful only when the team understands problems in {process} and works with them systematically',
    ],
  },
